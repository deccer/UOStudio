...
namespace UOStudio.TextureAtlasGenerator.Ultima
{
    ...
    public sealed class MultiComponentList
    {
            ...
            switch (type)
            {
                case Multis.ImportType.TXT:
                {
                    ...
                    using (var ip = new StreamReader(fileName))
                    {
                        ...
                        var centerX = _max.X - (int)(Math.Round((_max.X - _min.X) / 2.0));
                        var centerY = _max.Y - (int)(Math.Round((_max.Y - _min.Y) / 2.0));
        ...
        private void ConvertList()
        {
            ...
            {
                var xOffset = SortedTiles[i].m_OffsetX + _center.X;
                var yOffset = SortedTiles[i].m_OffsetY + _center.Y;
