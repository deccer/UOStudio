...
echo dotnet publish ${c} ${r} ${f} --self-contained=false -o dist/${os}-x64/client src/UOStudio.Client/UOStudio.Client.csproj
dotnet publish ${c} ${r} ${f} --self-contained=false -o dist/${os}-x64/client src/UOStudio.Client/UOStudio.Client.csproj
echo.
echo ==== Publishing Server ====
echo dotnet publish ${c} ${r} ${f} --self-contained=false -o dist/${os}-x64/server src/UOStudio.Server/UOStudio.Server.csproj
dotnet publish ${c} ${r} ${f} --self-contained=false -o dist/${os}-x64/server src/UOStudio.Server/UOStudio.Server.csproj
exit $?
